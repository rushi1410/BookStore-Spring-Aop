...
public class CartController {
    ...
    //localhost:8082/cart/add
    @PostMapping("/add")
    public ResponseEntity<ResponseDTO> addBook(@Valid @RequestBody CartDTO cartdto){
        ResponseDTO dto = new ResponseDTO("Book added In Cart successfully",cartService.addCart(cartdto));
        return new ResponseEntity<>(dto, HttpStatus.CREATED);
    }

    //Ability to call api to get all card records
    //localhost:8082/cart/getAllCarts
    ...
    //localhost:8082/cart/getCart/{id}
    ...
    //localhost:8082/cart/updateCart/{id}
    ...
    //localhost:8082/cart/updateQuantity/{id}/{quantity}
    ...
    //localhost:8082/cart/deleteCart/{id}
